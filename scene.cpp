...

{
  ...
  if (g == 1 || glossyIterations == 1) {
	...
	for (int i = 0; i < glossyIterations; i++){
		float radius = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/psi));
		float angle = rand() / (RAND_MAX + 1.0);
		angle = angle * 6.28;
		
		float x = radius * cos(angle);
		float y = radius * sin(angle);
		vec3 randPoint = P + vec3(x, y, 0);
		vec3 Rin = raytrace( randPoint, R, depth, objIndex, objPartIndex );
		Iout = Iout + calcIout( N, R, E, E, kd, mat->ks, mat->n, Rin );
	}
	Iout = vec3(Iout.x / glossyIterations, Iout.y / glossyIterations, Iout.z / glossyIterations);
  }
  ...
  for (int i=0; i<objects.size(); i++)
	...
	for(int i = 0;i < NUM_SOFT_SHADOW_RAYS;i++){
		...
		while(alpha + beta  > 1){
			alpha = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			beta = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
		}

		vec3 randomPoint = triangle->pointFromBarycentricCoords(1, alpha, beta) - P;
		vec3 Rin = raytrace( randomPoint, R, depth, objIndex, objPartIndex );
		Iout = Iout + calcIout( N, R, E, E, kd, mat->ks, mat->n, Rin );
	}
...

{
  ...
  // YOUR CODE HERE	
	...
	if (numPixelSamples > 1){
		for(int i = -numPixelSamples;i <= numPixelSamples;i++){
			for(int j = -numPixelSamples;j <= numPixelSamples;j++){
				if(jitter == 1){
					float xJitter = -1.0 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(2.0)));
					float yJitter = -1.0 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(2.0)));

					vec3 dir = (llCorner + (x + i + xJitter) * right + (y + j + yJitter) * up).normalize();
		...
		result = vec3(sum.x / numPixelSamples, sum.y / numPixelSamples, sum.z / numPixelSamples);
